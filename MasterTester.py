...
def RunTests(datasets, classifier, models, p):
    for dataset in datasets:
        dir = dataset.getdir()
        cb = MatrixGeneration.GenerateMatrix(dataset.getnumclasses(), dataset.getnumclasses())
        listOfCBs = [cb]
        labelcol = dataset.getlabelcol()
        begin = dataset.getbegin()
        end = dataset.getend()
        getECOCBaselineAccuracies(dir, listOfCBs, labelcol, begin, end, classifier)
        getECOCBaselineAccuracies_VD(dir, listOfCBs, labelcol, begin, end, classifier, p)
        getECOCBaselineAccuracies_VC(dir, listOfCBs, labelcol, begin, end, classifier, models)
        getECOCBaselineAccuracies_VC_VD(dir, listOfCBs, labelcol, begin, end, classifier, models, p)
...
cb = MatrixGeneration.GenerateMatrix(10, 10)


dataset = "/media/msaroshkhan/OS_Install/Users/M Sarosh Khan/PycharmProjects/CodebookTesting/datasets/pendigits.csv"

pendigits = DataSet("PenDigits", "/media/msaroshkhan/OS_Install/Users/M Sarosh Khan/PycharmProjects/CodebookTesting/datasets/pendigits.csv", -1, 0, 12, 10)
letters = DataSet("Letters", "/media/msaroshkhan/OS_Install/Users/M Sarosh Khan/PycharmProjects/CodebookTesting/datasets/LetterRecognition.csv", -1, 0, 15, 26)

DATASETS = []
DATASETS.append(pendigits)
DATASETS.append(letters)
...
'''
#getECOCBaselineAccuracies(dataset, listOfCBs, -1, 0, 15, 2)
#getECOCBaselineAccuracies(dataset, listOfCBs, -1, 0, 15, 4)
getECOCBaselineAccuracies(dataset, listOfCBs, -1, 0, 12, 7)
getECOCBaselineAccuracies_VC(dataset, listOfCBs, -1, 0, 12, models)


#getECOCBaselineAccuracies_VC(dataset, listOfCBs, -1, 0, 15, models)
'''
testerArray = [[],[],[],[],[]]
testerArrayCorr = [[],[],[],[]]
pdomain = []
while p >= 0.2:
    ...
    dt = getECOCBaselineAccuracies_VD(dataset, listOfCBs, -1, 0, 12, 2, p)
    testerArray[0].append(dt[0])
    testerArrayCorr[0].append(dt[1])

    knn = getECOCBaselineAccuracies_VD(dataset, listOfCBs, -1, 0, 12, 4, p)
    testerArray[1].append(knn[0])
    testerArrayCorr[1].append(knn[1])
    
    RF = getECOCBaselineAccuracies_VD(dataset, listOfCBs, -1, 0, 12, 7, p)
    testerArray[2].append(RF[0])
    testerArrayCorr[2].append(RF[1])

    mixed = getECOCBaselineAccuracies_VC_VD(dataset, listOfCBs, -1, 0, 12, models, p)
    testerArray[3].append(mixed[0])
    testerArrayCorr[3].append(mixed[1])
...
resplt.suptitle("Pendigits Varied Data Accuracies")
resplt.xlabel("Percent of Data Per Learner")
resplt.ylabel("Accuracy")
resplt.plot(pdomain, testerArray[0], '-b', label= "Decision Tree")
resplt.plot(pdomain, testerArray[1], '-r', label= "KNN")
resplt.plot(pdomain, testerArray[2], '-y', label= "Random Forest")
resplt.plot(pdomain, testerArray[3], '-g', label= "All")
resplt.legend(loc= "lower right")
resplt.show()
 
corrplt.suptitle("Pendigits Varied Data Correlation")
corrplt.xlabel("Percent of Data Per Learner")
corrplt.ylabel("Correlation")
corrplt.plot(pdomain, testerArrayCorr[0], '-b', label= "Decision Tree")
corrplt.plot(pdomain, testerArrayCorr[1], '-r', label= "KNN")
corrplt.plot(pdomain, testerArrayCorr[2], '-y', label= "Random Forest")
corrplt.plot(pdomain, testerArrayCorr[3], '-g', label= "All")
