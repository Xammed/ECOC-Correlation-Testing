...
class Trainer():
   ...
   # Return models so that predictions can be done later.
    ...
    def trainClassifiers_VD(self, knownData, knownLabels, model, p):
        ...
        for labels in knownLabels:
            ...
            if (p<0):
                ...
                pdata, plabels = knownData, knownLabels
            if model == 1:
                classifier = svm.SVC(gamma='auto')
            elif model == 2:
                classifier = DecisionTreeClassifier(random_state=0)
    ...
    def correlationMatrix(self, missclassifieds):
        matrix = []
        for i in range(len(missclassifieds)):
            matrix.append([])
            for j in range(len(missclassifieds)):
                #print(i, j)
                #print(matrix)
    ...
    def averageTotal(self, matrix):
        ...
        for i in range(len(matrix)):
            for j in range(len(matrix[0])):
                if i!=j:
                    average += matrix[i][j]
        return average/((len(matrix)-1)*len(matrix[0]))
...
def getECOCBaselineAccuracies_VC_VD(dataset, listOfCBs, labelCol, beginData, endData, modelChoices, p, f, graphing):
    ts = time.time()
    RunLabel = "VC_VD_"
    ...
    models = ["SVM", "DT", "LDA", "KNN",
          "LogisticRegression", "GaussianNB", "RandomForest"] # Used for printing
    ...
    for codebook in listOfCBs:
        ...
        if(graphing):
            for m in matrices:
                ...
                run_models = ""
                for mo in modelChoices:
                    run_models += models[mo-1]
                plt.savefig(dataset_str_name + "_" + run_models + ".png")
